...
PROJECT_OUT="$PROJECT" PATH="$bin_dir:$PATH" \
  "$AGENT_LSM_BIN" -d "$PROJECT" codex -- exec prompt
...
PROJECT_OUT="$PROJECT" PATH="$bin_dir:$PATH" \
  "$AGENT_LSM_BIN" --no-yolo -d "$PROJECT" codex -- exec prompt
codex_no_yolo=$(cat "$PROJECT/codex.args")
assert_not_contains "$codex_no_yolo" "--dangerously-bypass-approvals-and-sandbox"

mkdir -p "$XDG_CONFIG_HOME/agent-lsm"
printf 'DEFAULT_AGENT=gemini\n' > "$XDG_CONFIG_HOME/agent-lsm/config"
PROJECT_OUT="$PROJECT" PATH="$bin_dir:$PATH" \
  "$AGENT_LSM_BIN" -d "$PROJECT" -- --prompt hi
