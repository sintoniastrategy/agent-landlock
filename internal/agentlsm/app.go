...
func (a App) bootstrap(inv Invocation) (int, error) {
	if inv.Undo {
		fmt.Fprintln(a.Stdout, "bootstrap is not required for agent-lsm; no paired user or sudoers dropin exists")
		return ExitOK, nil
	}
	if inv.Common.DryRun {
		dir, _ := stateDir()
...
func helpText() string {
	...
	sort.Strings(cmds)
	return fmt.Sprintf(`agent-lsm: run AI agents under a Linux Landlock write boundary

usage:
  agent-lsm [FLAGS] [claude|codex|gemini] [-- AGENT_ARGS...]
  agent-lsm [FLAGS] run -- CMD ARGS...
  agent-lsm grant PATH [--timeout=30m]
  agent-lsm revoke PATH
  agent-lsm grants [--cleanup]
  agent-lsm status [DIR]
  agent-lsm doctor [--heal] [DIR]
...
commands: %s
state:    ~/.local/state/agent-lsm/
config:   /etc/agent-lsm/config, ~/.config/agent-lsm/config
`, strings.Join(cmds, ", "))
}
