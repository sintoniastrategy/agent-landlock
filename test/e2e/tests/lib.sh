...
require_landlock() {
  if ! "$AGENT_LANDLOCK_BIN" doctor "$PROJECT" >/tmp/agent-landlock-doctor.out 2>&1; then
    cat /tmp/agent-landlock-doctor.out >&2 || true
    fail "Landlock ABI v3+ is required for agent-landlock e2e tests"
  fi
}
